...
namespace IS_Project
{
    class Diagram
    {
        ...
        public int DFSLowerValues(int idx)
        {
            int T;
            if (node[idx].vis == 1)
            {
            ...
            node[idx].vis = 1;
            int minLS = 1000000;
            List<int> revEdge = rEdges[idx];
            if (revEdge.Count == 0)
            {
                node[idx].LF = node[idx].EF;
                node[idx].calculateLS();
                node[idx].V2 = node[idx].LF - node[idx].EF;
                node[idx].V1 = node[idx].LS - node[idx].ES;
                return node[idx].LS;
            }
            for (int i = 0; i < revEdge.Count; i++)
            {
                int currLF;
                currLF = DFSLowerValues(revEdge[i]);
                minLS = Math.Min(minLS, currLF);
            }
            node[idx].LF = minLS;
            node[idx].calculateLS();
            node[idx].V2 = node[idx].LF - node[idx].EF;
        ...
        public void drawDiagram()
        {
            ...
            addWidth = Math.Max(drawingForm.Width / (numberOfNodes + 2) , SIZE.Width);
        ...
        private void panel_paint(object sender, PaintEventArgs e)
        {
            ...
            Pen CriticalPen = new Pen(Color.Red);
            Pen CriticalPenArrow = new Pen(Color.Red);
            CriticalPen.DashStyle = DashStyle.Dash;
            CriticalPenArrow.DashStyle = DashStyle.Dash;
            ...
            using (GraphicsPath capPath = new GraphicsPath())
            {
                ...
                CriticalPenArrow.CustomEndCap = new CustomLineCap(null, capPath);
                ...
                foreach(int child in topoSort)
                {
                    ...
                    {
                        Pen temp = penWithArrow;
                        Pen temp2 = penWithoutArrow;
                        if(node[child].isCritical)
                        {
                            penWithArrow = CriticalPenArrow;
                            penWithoutArrow = CriticalPen;
                        }
                        int i = 0;
                        ...
                        penWithArrow = temp;
                        penWithoutArrow = temp2;
                    }

                    foreach(int parent in node[child].child())
                    {
                        node[parent].isFree = false;
                        Pen temp = penWithArrow;
                        Pen temp2 = penWithoutArrow;
